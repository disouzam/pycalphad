...
#git cherry-pick -x --no-commit 5f28b504c #Build(deps): Bump pypa/cibuildwheel from 4.0.0 to 4.1.0 (#691)
#git cherry-pick -x --no-commit 6347c9bfc #TST: Add test that state variable units survive roundtrip pickle (#688)
#git cherry-pick -x --no-commit 29d9fe273 #First version of `run_pytest_and_capture.py`
#git cherry-pick -x --no-commit 2242f6a1e #Modification to add timestamp to all output lines
#git cherry-pick -x --no-commit 632e178c3 #Debug programmatically pytest execution
#git cherry-pick -x --no-commit 1f2bf7628 #Revert "Modification to add timestamp to all output lines"
#git cherry-pick -x --no-commit 9813d3598 #Added function to read ini options from pyproject.toml
#git cherry-pick -x --no-commit cdd4b080e #Fixed formatting of session output
#git cherry-pick -x --no-commit e52604ae6 #Added another launch configuration to debug pytest execution via run_pytest_and_capture.py with args
#git cherry-pick -x --no-commit b12f07e07 #Parse args from command line and passed them ipsis literis from pytest
#git cherry-pick -x --no-commit 74f0c82c7 #Added a new parameter to be unique for `run_pytest_and_capture.py` and `conftest.py` and be uused as a reference time stamp for file name composition
#git cherry-pick -x --no-commit f45bb1382 #Adjusting session file name to use the same start time stamp as the one used in report file
#git cherry-pick -x --no-commit a0fd51215 #Added workspace file (for VS Code) to deal with PyCalphad and Cosmic-Ray in same instance
#git cherry-pick -x --no-commit 14e80bdc8 #Adjusted timeout to match the total length of a typical test execution (75 seconds) and have a small margin to cope with longer loops
#git cherry-pick -x --no-commit 6c0fad2c7 #Set exit code in run_pytest_and_capture to correctly classify mutants
#git cherry-pick -x --no-commit 2814bb0ee #Added configuration to save HTML reports for each mutant
#git cherry-pick -x --no-commit 03b63ee93 #Modified verbosity and fixed path to Local_distributor folder in configuration to debug Cosmic Ray in launch.json
#git cherry-pick -x --no-commit d1ade090b #Hide pytest ini options from session output
#git cherry-pick -x --no-commit 0299dd77c #Added `--job-id` as a parameter to pytest
#git cherry-pick -x --no-commit 6ba4524b3 #Retrieval of pytest configuration inside main script that runs pytest
#git cherry-pick -x --no-commit 136a53a80 #Added condition for missing job ids
#git cherry-pick -x --no-commit b384d3f8a #Removed extra print statement in conftest that ends up in session output
#git cherry-pick -x --no-commit 14b2a968c #Modified Cosmic Ray configuration for 2 and 10 workers
#git cherry-pick -x --no-commit 21787e0f8 #Increased timeout from 15 seconds (wrong value) to 90 seconds (20% higher than the normal execution of test suite
#git cherry-pick -x --no-commit 0416e86b0 #Restarted / reinitialized database for 10 workers setup
#git cherry-pick -x --no-commit 65e4985f6 #Adjusted source of Cosmic-Ray to enable different clones in the same machine without disturbing pycalphad mutation testing
#git cherry-pick -x --no-commit 9524d6127 #Database after ~50% of mutants tests but with several errors in Cosmic Ray workers
#git cherry-pick -x --no-commit c4c4f37a3 #Converted configuration of `mutation_testing/10_workers/cosmic_ray.toml` to use a single distributor instead of 10 paralell http workers
#git cherry-pick -x --no-commit 91977593e #Short-lived execution running in a single distributor
#git cherry-pick -x --no-commit 13a2cb2fa #Restarted database for local distributor
#git cherry-pick -x --no-commit 4b880354d #Partial results of mutation testing - around 6%
#git cherry-pick -x --no-commit 3da2e85aa #Partial update of local distributor with 3574 mutants already checked, with 1101 surviving mutants
#git cherry-pick -x --no-commit 095c7d60f #Partial update of local distributor with 15433 mutants already checked, with 4748 surviving mutants
#git cherry-pick -x --no-commit 3b942284d #Set verbosity of local distributor launcher of Cosmic Ray to be DEBUG
#git cherry-pick -x --no-commit 163988cdb #Updated source path to Cosmic Ray to use a dedicated version of Cosmic Ray for PyCalphad
#git cherry-pick -x --no-commit 32c3e49a6 #Partial update of local distributor with 15483 mutants already checked, with 4765 surviving mutants
#git cherry-pick -x --no-commit 0dd5d1863 #Script to monitor status of mutation testing periodically and emit a sound when mutation testing stalls
#git cherry-pick -x --no-commit 162d73daa #Removed Cosmic-Ray reports
